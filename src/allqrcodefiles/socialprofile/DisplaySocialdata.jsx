...
const DisplaySocialData = () => {
  ...
  const fetchSocialData = async (config) => {
    try {
      ...
      const businessResponse = await axios.get(`${process.env.REACT_APP_BACKEND_URL}/diplay/qr/data/social/${qrId}`);
        ...
        <div>
          {businessDataGet ? (
            <div className='small-preview-data-from-scanning'>
                <SocialProfileDisplay ConfigurationSocialData={ConfigurationData} socialDataGet={businessDataGet}  />
            </div>
          ) : (
            <div>{error}</div>
...
export default DisplaySocialData;
