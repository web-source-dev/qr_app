...
import MusicDisplay from './MobileMusicDisplay';
...
const DisplayImages = () => {
  ...
  const fetchSocialData = async (config) => {
    try {
      ...
      const businessResponse = await axios.get(`${process.env.REACT_APP_BACKEND_URL}/diplay/qr/data/music/${qrId}`);
        ...
        <div>
          {businessDataGet ? (
            <div className='small-preview-data-from-scanning'>
                <MusicDisplay ConfigurationShopMenuData={ConfigurationData} shopMenuDataGet={businessDataGet}  />
            </div>
          ) : (
            <div>{error}</div>
