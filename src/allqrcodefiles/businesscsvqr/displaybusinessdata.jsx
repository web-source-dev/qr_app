...
import Timeschedule from './themes/days-current';
...
const DisplayBusinessData = () => {
    ...
    const fetchConfigData = async () => {
      try {
        ...
        if (configResponse.data.config?.active_password) {
          ...
          fetchBusinessData(configResponse.data.config);
  ...
  const fetchBusinessData = async (config) => {
    try {
      if (!config?.qr_active) {
        setError('QR code is inactive.');
        return;
      ...
      const businessResponse = await axios.get(`${process.env.REACT_APP_BACKEND_URL}/diplay/qr/data/business/${qrId}`);
      setBusinessData(businessResponse.data.BusinessDataSend);
      localStorage.setItem("afterScan", "true");
      console.log('Business data fetched:', businessResponse.data.BusinessDataSend);
    } catch (error) {
        ...
        <div>
          {businessDataGet ? (
            <div className='small-preview-data-from-scanning'>
                <DefualtBusinessTheme ConfigurationData={ConfigurationData} businessDataGet={businessDataGet}  />
            </div>
          ) : (
            <div>{error}</div>
...
export default DisplayBusinessData;
