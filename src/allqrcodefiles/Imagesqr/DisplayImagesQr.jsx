...
const DisplayImages = () => {
  ...
  const fetchSocialData = async (config) => {
    try {
      ...
      const businessResponse = await axios.get(`${process.env.REACT_APP_BACKEND_URL}/diplay/qr/data/images/${qrId}`);
        ...
        <div>
          {businessDataGet ? (
            <div className='small-preview-data-from-scanning'>
                <ImagesDisplay ConfigurationShopMenuData={ConfigurationData} shopMenuDataGet={businessDataGet}  />
            </div>
          ) : (
            <div>{error}</div>
